...
def catalog_mock(auth_mock, requests_mock):
    ...
    with open(
        pathlib.Path(__file__).resolve().parents[1] / "mock_data/data_products.json", encoding="utf-8"
    ) as json_file:
    ...
    with open(
        pathlib.Path(__file__).resolve().parents[1] / "mock_data/search_response.json", encoding="utf-8"
    ) as json_file:
    ...
    with open(
        pathlib.Path(__file__).resolve().parents[1] / "mock_data/data_product_spot_schema.json", encoding="utf-8"
    ) as json_file:
