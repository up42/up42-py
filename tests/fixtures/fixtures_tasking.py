...
def tasking_mock(auth_mock, requests_mock):
    ...
    url_get_quotations_mp1 = f"{auth_mock._endpoint()}/v2/tasking/quotation?page=0&sort=createdAt,desc"
    with open(
        Path(__file__).resolve().parents[1]
    ...
    url_get_quotations_mp2 = f"{auth_mock._endpoint()}/v2/tasking/quotation?page=1&sort=createdAt,desc"
    with open(
        Path(__file__).resolve().parents[1]
    ...
    url_get_quotations_mp3 = f"{auth_mock._endpoint()}/v2/tasking/quotation?page=2&sort=createdAt,desc"
    with open(
        Path(__file__).resolve().parents[1]
