...
def tasking_mock(auth_mock, requests_mock):
    ...
    url_get_quotations_mp1 = (
        f"{API_HOST}{QUOTATION_ENDPOINT}?page=0&sort=createdAt,desc"
    )
    with open(
        Path(__file__).resolve().parents[1]
        ...
        requests_mock.get(
            url=url_get_quotations_mp1, json=json_data_get_quotation
        )

    url_get_quotations_mp2 = (
        f"{API_HOST}{QUOTATION_ENDPOINT}?page=1&sort=createdAt,desc"
    )
    with open(
        Path(__file__).resolve().parents[1]
        ...
        json_data_get_quotation = json.load(json_file)
        requests_mock.get(
            url=url_get_quotations_mp2, json=json_data_get_quotation
        )

    url_get_quotations_mp3 = (
        f"{API_HOST}{QUOTATION_ENDPOINT}?page=2&sort=createdAt,desc"
    )
    with open(
        Path(__file__).resolve().parents[1]
        ...
        json_data_get_quotation = json.load(json_file)
        requests_mock.get(
            url=url_get_quotations_mp3, json=json_data_get_quotation
        )
    ...
    url_get_quotations_decision_filtered = (
        f"{API_HOST}{QUOTATION_ENDPOINT}?{sorting}{decision_filter}"
    )
    with open(
        Path(__file__).resolve().parents[1]
        / "mock_data/tasking_data/get_quotations_decision_ACCEPTED.json"
...
def tasking_get_feasibility_mock(auth_mock, requests_mock):
    get_feasibility_page0_url = (
        f"{API_HOST}/v2/tasking/feasibility?page=0&sort=createdAt,desc"
    )
    with open(
        Path(__file__).resolve().parents[1]
    ...
    get_feasibility_page1_url = (
        f"{API_HOST}/v2/tasking/feasibility?page=1&sort=createdAt,desc"
    )
    with open(
        Path(__file__).resolve().parents[1]
    ...
    get_feasibility_page2_url = (
        f"{API_HOST}/v2/tasking/feasibility?page=2&sort=createdAt,desc"
    )
    with open(
        Path(__file__).resolve().parents[1]
    ...
    get_feasibility_decision_param = (
        f"{API_HOST}/v2/tasking/feasibility?"
...
def tasking_choose_feasibility_mock(auth_mock, requests_mock):
    wrong_feasibility_url = (
        f"{API_HOST}/v2/tasking/feasibility/{WRONG_FEASIBILITY_ID}"
    )
    response = json.dumps(
        {
            "status": 404,
    ...
    requests_mock.patch(
        url=wrong_feasibility_url, status_code=404, json=response
    )

    choose_feasibility_url = (
        f"{API_HOST}/v2/tasking/feasibility/{LIVE_FEASIBILITY_ID}"
    )
    response = json.dumps(
        {
            "status": 405,
    ...
    requests_mock.patch(
        url=choose_feasibility_url, status_code=405, json=response
    )
    return Tasking(auth=auth_mock)
