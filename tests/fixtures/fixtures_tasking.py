...
def tasking_mock(auth_mock, requests_mock):
    ...
    url_get_quotations_workspace_filtered = f"{auth_mock._endpoint()}{endpoint}{sorting}&workspaceId={workspace}"
    with open(
        Path(__file__).resolve().parents[1]
    ...
    url_get_quotations_decision_filtered = f"{auth_mock._endpoint()}{endpoint}{sorting}&decision={decision}"
    with open(
        Path(__file__).resolve().parents[1]
