...
def job_mock(auth_mock, requests_mock):
    ...
    # get_jobtasks
    url_job_tasks = (
        f"{API_HOST}/projects/{job.project_id}/jobs/{job.job_id}/tasks/"
    )
    requests_mock.get(url=url_job_tasks, json={"data": [{"id": JOBTASK_ID}]})

    # get_logs
    url_logs = (
        f"{API_HOST}/projects/{job.project_id}/jobs/"
    ...
    # quicklooks
    url_quicklook = (
        f"{API_HOST}/projects/{job.project_id}/jobs/{job.job_id}"
    ...
    # get_job_credits
    url_download_result = (
        f"{API_HOST}/projects/" f"{job.project_id}/jobs/{job.job_id}/credits"
    )
    requests_mock.get(
        url_download_result,
        json=MOCK_CREDITS,
