...
class TestBuildMessage:
    def test_should_build_warn_message(self):
        assert (
            version_control.build_outdated_version_message(
                fake_installed_version, fake_latest_version
            )
...
class TestCheckLatestVersion:
    def test_should_warn_if_not_latest_version(
        ...
        warn = mock.MagicMock()
        requests_mock.get(
            url="https://pypi.org/pypi/up42-py/json",
            json={"info": {"version": fake_latest_version}},
        )
        ...
        build_warning_message.assert_called_with(
            fake_installed_version, version.Version(fake_latest_version)
        )
    ...
    def test_should_not_check_the_version_if_latest_version_check_is_disabled(
        self,
    ):
        unused = mock.MagicMock()
...
class TestIsLatestVersionCheckEnabled:
    ...
    def test_should_return_correct_boolean_for_valid_values(
        self, env_var, expected_result
    ):
        get_env_var = mock.MagicMock(return_value=env_var)
        assert (
            version_control.is_latest_version_check_enabled(
                get_environment_variable=get_env_var
            )
            is expected_result
        )
