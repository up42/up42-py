...
def test_global_auth_initialize_objects(
    ...
    up42_project = up42.initialize_project()
    assert isinstance(up42_project, project.Project)
    up42_catalog = up42.initialize_catalog()
    assert isinstance(up42_catalog, catalog.Catalog)
    up42_workflow = up42.initialize_workflow(workflow_id=constants.WORKFLOW_ID)
    assert isinstance(up42_workflow, workflow.Workflow)
    up42_job = up42.initialize_job(job_id=constants.JOB_ID)
    assert isinstance(up42_job, job.Job)
    up42_jobtask = up42.initialize_jobtask(job_id=constants.JOB_ID, jobtask_id=constants.JOBTASK_ID)
    assert isinstance(up42_jobtask, job.JobTask)
    up42_jobcollection = up42.initialize_jobcollection(job_ids=[constants.JOB_ID, constants.JOB_ID])
    assert isinstance(up42_jobcollection, jobcollection.JobCollection)
    up42_storage = up42.initialize_storage()
    assert isinstance(up42_storage, storage.Storage)
    up42_order = up42.initialize_order(order_id=constants.ORDER_ID)
    assert isinstance(up42_order, order.Order)
    up42_asset = up42.initialize_asset(asset_id=constants.ASSET_ID)
...
def test_should_initialize_workflow(auth_mock, requests_mock):
    ...
    workflow = up42.initialize_workflow(constants.WORKFLOW_ID, project_id)
...
def test_should_initialize_workflow_with_implicit_project_id(auth_mock, workflow_mock):
    ...
    workflow = up42.initialize_workflow(constants.WORKFLOW_ID)
...
def test_should_initialize_job(auth_mock, requests_mock):
    ...
    job = up42.initialize_job(constants.JOB_ID, project_id)
...
def test_should_initialize_job_with_implicit_project_id(auth_mock, job_mock):
    ...
    job = up42.initialize_job(constants.JOB_ID)
...
def test_should_initialize_jobtask(auth_mock, requests_mock, jobtask_mock):
    ...
    job = up42.initialize_jobtask(constants.JOBTASK_ID, constants.JOB_ID, project_id)
    assert job.project_id == project_id
...
def test_should_initialize_job_task_with_implicit_project_id(auth_mock, jobtask_mock):
    ...
    job = up42.initialize_jobtask(constants.JOBTASK_ID, constants.JOB_ID)
    assert job.project_id == constants.PROJECT_ID
