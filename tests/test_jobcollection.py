...
def test_job_iterator(
    ...
    res = jobcollection_multiple_mock.apply(
        worker_with_add, add=5, only_succeeded=False
    )
    assert len(res) == 2
    assert res[JOB_ID] == 5
    ...
    res = jobcollection_multiple_mock.apply(
        worker_with_add, add=5, only_succeeded=True
    )
    assert len(res) == 1
    assert res[JOB_ID_2] == 5

    with pytest.raises(ValueError) as e:
        jobcollection_empty_mock.apply(
            worker_with_add, add=5, only_succeeded=True
        )
        assert (
            str(e)
...
def test_jobcollection_info(jobcollection_single_mock, requests_mock):
    ...
    requests_mock.get(
        url=url_job_info, json={"data": {"xyz": 789}, "error": {}}
    )
...
def test_jobcollection_jobs_status(
...
def test_jobcollection_download_results(
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        out_dict = jobcollection_single_mock.download_results(
            tmpdir, merge=False
        )
...
def test_jobcollection_download_results_merged(
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        out_dict = jobcollection_multiple_mock.download_results(
            tmpdir, merge=True
        )
        print(out_dict)
        ...
        with open(out_dict["merged_result"][0]) as src:
            ...
            assert (
                tmpdir
                / Path(
                    merged_data_json.features[0].properties["up42.data_path"]
                )
            ).exists()
