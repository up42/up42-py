...
TOKEN_SETTINGS = config.TokenProviderSettings(
    token_url=TOKEN_URL, timeout=HTTP_TIMEOUT, expiry_offset=1
)
...
def _token():
    return oauth.Token(
        ...
        + datetime.timedelta(
            seconds=TOKEN_EXPIRES_IN - TOKEN_SETTINGS.expiry_offset
        ),
    )
...
class TestAccountTokenRetriever:
    ...
    def test_fails_to_retrieve_for_bad_response(
...
class TestUp42Auth:
    def test_should_fetch_token_when_created(self, token):
        ...
        up42_auth = oauth.Up42Auth(
            retrieve=retrieve, token_settings=TOKEN_SETTINGS
        )
    ...
    def test_should_fetch_token_when_expired(self, token):
        second_token = oauth.Token(
            access_token="token2", expires_on=datetime.datetime.max
        )
        ...
        assert (
            ...
            == f"Bearer {second_token.access_token}"
        )
        assert TOKEN_SETTINGS == retrieve.call_args.args[1]
...
class TestDetectRetriever:
    def test_should_detect_account_retriever(self):
        assert isinstance(
            oauth.detect_retriever(ACCOUNT_CREDENTIALS),
            oauth.AccountTokenRetriever,
        )
