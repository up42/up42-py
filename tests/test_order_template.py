...
class TestBatchOrderTemplate:
    ...
    @pytest.mark.parametrize(
        "title",
        [
            "EULA 95d6fff5-493c-4118-8edf-3096e73eda12 is not accepted",
            "Access to collection pixxel-tasking is restricted for account 3c86279b-610a-4dba-ad12-abe3bdc51428.",
            "other text",
        ],
    )
    def test_should_raise_legal_reasons_error(self, requests_mock: req_mock.Mocker, title: str):
        ...
        with pytest.raises(order_template.LegalReasonsError) as exc_info:
            template.place()

        assert title in str(exc_info.value)

    @pytest.mark.parametrize("status_code", [400, 401, 403, 404, 500, 503])
    def test_should_handle_other_http_errors(self, requests_mock: req_mock.Mocker, status_code: int):
        estimate_url = f"{constants.API_HOST}/v2/orders/estimate"
        requests_mock.post(url=estimate_url, json=ESTIMATE_PAYLOAD)
        placement_url = f"{constants.API_HOST}/v2/orders?workspaceId={constants.WORKSPACE_ID}"
        requests_mock.post(url=placement_url, status_code=status_code)
