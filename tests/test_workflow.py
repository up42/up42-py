...
from .fixtures import api_mock, workflow_mock
import up42


def test_workflow_get_info(workflow_mock):
    with requests_mock.Mocker() as m:
        url_workflow_info = f"{workflow_mock.api._endpoint()}/projects/{workflow_mock.project_id}/workflows/{workflow_mock.workflow_id}"
        m.get(url=url_workflow_info, text='{"data": {"xyz":789}, "error":{}}')

        info = workflow_mock._get_info()
    assert isinstance(workflow_mock, up42.Workflow)
    assert info["xyz"] == 789
...
def test_get_workflow_tasks(workflow_mock):
    with requests_mock.Mocker() as m:
        url_workflow_tasks = (
            f"{workflow_mock.api._endpoint()}/projects/{workflow_mock.api.project_id}/workflows/"
