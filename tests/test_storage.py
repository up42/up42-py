...
def test_get_assets_pagination(auth_mock, requests_mock):
    ...
    requests_mock.get(
        url=url_storage_assets_paginated, json=json_assets_paginated
    )
...
def test_get_orders_v2_endpoint_params(
    ...
    endpoint_statuses = (
        set(params["statuses"]) & allowed_statuses
        if params["statuses"]
        else []
    )
    url_params = "&".join(
...
def test_get_orders_pagination(auth_mock, requests_mock):
    ...
    requests_mock.get(
        url=url_storage_orders_paginated, json=json_orders_paginated
    )
